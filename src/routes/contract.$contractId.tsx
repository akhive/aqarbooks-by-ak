...
function ContractDetailPage() {
  ...
  const { data, loading, refresh, addCheque, deleteCheque, updateContract, addContract } = store;

  const contract = data.contracts.find((c: any) => c.id === contractId);
  const tenant = data.tenants.find((t: any) => t.id === contract?.tenantId);
  const unit = data.units.find((u: any) => u.id === contract?.unitId);
  ...
  const submitDraft = async () => {
    ...
    try {
      ...
      toast.success("Lease submitted — now Active");
      await refresh();
    } catch (e: any) {
  ...
  const chequeTotal = cheques.reduce((s: number, c: any) => s + (c.amount || 0), 0);
        ...
        <DialogContent>
          ...
          <div className="space-y-3">
            ...
            <p className="text-xs text-muted-foreground">
              Creates a <strong>Draft</strong> lease. Use <strong>Submit lease</strong> when ready.
            </p>
          </div>
          <DialogFooter>
